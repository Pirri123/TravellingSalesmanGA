...
void normalize(int population, int cities, int popArray[population][cities], int evalArray[population], double popProba[population]){
    int fitness;
    int i, j;
    int distance = 0;
    ...
    for (i = 0; i < population; i++){
        for (j = 1; j < cities; j++ ){
            int startPoint = popArray[i][j-1];
            int endPoint= popArray[i][j];
            distance += map[startPoint][endPoint];

        }
        evalArray[i] = distance;
        distance = 0;
    }
...
int main( ) {
   ...
   //int popArray [10][5] = {0};
   int evalArray [population]; //En donde se guardan los resultados de la population al evaluarse y tambien donde se guarda la probabilidad de cada elemento de la population en ser escogido para futuras populationes
   double popProba[population];
   ...
   int k;
    ...
    for (i = 0; i < 1; i++){ //generations
        for (j = 0; j < population; j++){
            generateRoads(population, cities, popArray, j);
   }
        ...
        printf("\n\nNormalized:\n ");
        normalize(population, cities, popArray, evalArray, popProba);
